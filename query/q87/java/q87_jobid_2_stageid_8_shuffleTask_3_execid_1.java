/* 001 */ public java.lang.Object generate(Object[] references) {
/* 002 */   return new SpecificUnsafeProjection(references);
/* 003 */ }
/* 004 */
/* 005 */ class SpecificUnsafeProjection extends org.apache.spark.sql.catalyst.expressions.UnsafeProjection {
/* 006 */
/* 007 */   private Object[] references;
/* 008 */   private int value1;
/* 009 */   private UnsafeRow result;
/* 010 */   private org.apache.spark.sql.catalyst.expressions.codegen.BufferHolder holder;
/* 011 */   private org.apache.spark.sql.catalyst.expressions.codegen.UnsafeRowWriter rowWriter;
/* 012 */
/* 013 */   public SpecificUnsafeProjection(Object[] references) {
/* 014 */     this.references = references;
/* 015 */
/* 016 */     result = new UnsafeRow(1);
/* 017 */     this.holder = new org.apache.spark.sql.catalyst.expressions.codegen.BufferHolder(result, 0);
/* 018 */     this.rowWriter = new org.apache.spark.sql.catalyst.expressions.codegen.UnsafeRowWriter(holder, 1);
/* 019 */
/* 020 */   }
/* 021 */
/* 022 */   public void initialize(int partitionIndex) {
/* 023 */
/* 024 */   }
/* 025 */
/* 026 */
/* 027 */   private void apply_1(InternalRow i) {
/* 028 */
/* 029 */     boolean isNull9 = i.isNullAt(2);
/* 030 */     UTF8String value9 = isNull9 ? null : (i.getUTF8String(2));
/* 031 */     boolean isNull8 = isNull9;
/* 032 */     UTF8String value8 = value9;
/* 033 */     if (isNull8) {
/* 034 */
/* 035 */       Object obj2 = ((Expression) references[2]).eval(null);
/* 036 */       UTF8String value10 = (UTF8String) obj2;
/* 037 */       if (!false) {
/* 038 */         isNull8 = false;
/* 039 */         value8 = value10;
/* 040 */       }
/* 041 */     }
/* 042 */     value1 = org.apache.spark.unsafe.hash.Murmur3_x86_32.hashUnsafeBytes(value8.getBaseObject(), value8.getBaseOffset(), value8.numBytes(), value1);
/* 043 */   }
/* 044 */
/* 045 */
/* 046 */   private void apply_0(InternalRow i) {
/* 047 */
/* 048 */     boolean isNull3 = i.isNullAt(0);
/* 049 */     UTF8String value3 = isNull3 ? null : (i.getUTF8String(0));
/* 050 */     boolean isNull2 = isNull3;
/* 051 */     UTF8String value2 = value3;
/* 052 */     if (isNull2) {
/* 053 */
/* 054 */       Object obj = ((Expression) references[0]).eval(null);
/* 055 */       UTF8String value4 = (UTF8String) obj;
/* 056 */       if (!false) {
/* 057 */         isNull2 = false;
/* 058 */         value2 = value4;
/* 059 */       }
/* 060 */     }
/* 061 */     value1 = org.apache.spark.unsafe.hash.Murmur3_x86_32.hashUnsafeBytes(value2.getBaseObject(), value2.getBaseOffset(), value2.numBytes(), value1);
/* 062 */     boolean isNull6 = i.isNullAt(1);
/* 063 */     UTF8String value6 = isNull6 ? null : (i.getUTF8String(1));
/* 064 */     boolean isNull5 = isNull6;
/* 065 */     UTF8String value5 = value6;
/* 066 */     if (isNull5) {
/* 067 */
/* 068 */       Object obj1 = ((Expression) references[1]).eval(null);
/* 069 */       UTF8String value7 = (UTF8String) obj1;
/* 070 */       if (!false) {
/* 071 */         isNull5 = false;
/* 072 */         value5 = value7;
/* 073 */       }
/* 074 */     }
/* 075 */     value1 = org.apache.spark.unsafe.hash.Murmur3_x86_32.hashUnsafeBytes(value5.getBaseObject(), value5.getBaseOffset(), value5.numBytes(), value1);
/* 076 */   }
/* 077 */
/* 078 */
/* 079 */   // Scala.Function1 need this
/* 080 */   public java.lang.Object apply(java.lang.Object row) {
/* 081 */     return apply((InternalRow) row);
/* 082 */   }
/* 083 */
/* 084 */   public UnsafeRow apply(InternalRow i) {
/* 085 */     boolean isNull = false;
/* 086 */
/* 087 */     value1 = 42;
/* 088 */     apply_0(i);
/* 089 */     apply_1(i);
/* 090 */
/* 091 */     int value = -1;
/* 092 */
/* 093 */     int remainder = value1 % 64;
/* 094 */     if (remainder < 0) {
/* 095 */       value = (remainder + 64) % 64;
/* 096 */     } else {
/* 097 */       value = remainder;
/* 098 */     }
/* 099 */     rowWriter.write(0, value);
/* 100 */     return result;
/* 101 */   }
/* 102 */ }
